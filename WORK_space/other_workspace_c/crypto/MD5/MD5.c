...
#else
typedef int long32;           /* In 64-bit systems, long is 64-bit, */
typedef unsigned int ulong32; /* here we force ulong32 to be 32-bit. */
#endif
...
int Update_MD5(MD5_CTX *MD5_ctx, unsigned char *buffer, ulong32 count);
int Final_MD5(MD5_CTX *MD5_ctx);
static void Process_One_Block_MD5 (ulong32 state[4], unsigned char block[64]);
...
#define GG(a, b, c, d, x, shift, const_sin) { \
   (a) += G((b), (c), (d)) + (x) + (ulong32)(const_sin); \
   (a) = ROL((a), (shift)); \
   (a) += (b); \
   }

#define HH(a, b, c, d, x, shift, const_sin) { \
   (a) += H((b), (c), (d)) + (x) + (ulong32)(const_sin); \
   (a) = ROL((a), (shift)); \
   (a) += (b); \
   }

#define II(a, b, c, d, x, shift, const_sin) { \
   (a) += I((b), (c), (d)) + (x) + (ulong32)(const_sin); \
   (a) = ROL((a), (shift)); \
   (a) += (b); \
   }
               ...
               ulong32 buf_len)
{
   ...
   //*(unsigned __int64 *)MD5_ctx->count 
   //   += (unsigned __int64)buf_len << 3; // 更新报文位数
   MD5_ctx->count[0] += buf_len<<3;
   if(MD5_ctx->count[0] < buf_len<<3)
      MD5_ctx->count[1]++;
...
static void Process_One_Block_MD5(ulong32 state[4], unsigned char block[64])
{
   ulong32 a = state[0], b = state[1], c = state[2], d = state[3];
   ulong32 *px = (ulong32 *)block;
