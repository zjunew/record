...
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4])
{
   ...
   for(int i = 0 ; i < 4 ; i++)
   {
      for(int j = 0 ; j < 4 ;j++)
      {
         help[i] ^= aes_8bit_mul_mod_0x11B ( temp[j] , y [j]) ;
          if(i == 1&&flag)
          {
              printf("\n");
              printf("%x %x " , temp [j] , y [j]);
              printf("\n");
          }
      }
      rol_a_row(temp , 1);
    ...
    if(flag)
    {
      ...
      unsigned int c = 0x03 ;
      unsigned int d = 0xb2 ; 
      printf("%x " , aes_8bit_mul_mod_0x11B ( c , d ) );
      printf("\n");
        for(int k = 0 ; k < 4 ; k++)
        {
            printf("%x " ,y [k] );
...
void aes_decrypt(unsigned char *bufin, unsigned char *bufout, unsigned char *key)
{
   ...
   memcpy(matrix, bufin, 4*4); /* 复制明文16字节到matrix */
    ...
    //     printf("%x %x %x %x \n", bufin[k],bufin[k+1],bufin[k+2] ,bufin[k+3]);
    //     printf("\n");
    // }
